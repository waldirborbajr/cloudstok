...
openssl req -new -x509 -days 365 -key ca.key -out cacert.pem -subj "/C=IN/ST=NSW/L=NFeLoader/O=WaldirBorbaJR/OU=Org/CN=RootCA"
...
openssl req -new -key server.key -out server.csr -subj "/C=IN/ST=NSW/L=NFeLoader/O=WaldirBorbaJR/OU=Org/CN=ubuntu"
